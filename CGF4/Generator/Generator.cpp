...
void drawPlano(int len, int div) {
    ...
    fstream(file);
    file.open("C:\\Users\\Pestana\\Desktop\\CGF3\\3dFiles\\" + fileName, fstream::out); //abre ficheiro para escrita
    if (file.is_open()) {
...
Ponto calcula(std::vector<int> patch, std::vector<Ponto*> pontos, float u, float v) {
    ...
    //Calcula um vertice da curva segundo pontos de controlo
...
void drawBezierPatches(int nivel, string origem) {
    ...
    //abrir o ficheiro
    ifstream file1("C:\\Users\\Pestana\\Desktop\\CGF3\\Patches\\" + origem + ".patch");
    ...
    for (int i = 0; i < nPatches; i++) {
        ...
        for (int j = 0; j < nivel; j++) {
            for (int m = 0; m < nivel; m++) {
                //O resultado sao os vertices calculados que ja temos
                //U calcula o intervalo em x e V em Y
...
int  main(int argc, char* argv[]) {
    ...
    if ((strcmp(argv[1], "bezier") == 0 || strcmp(argv[1], "patch") == 0)  && argc == 5) {
